...
#    USB_CHAT          = Enable generation of USB chat file
#    USB_CHATFILE      = USB chatfile
#    
#
...
generate_lighttpd_env() {
	...
	local SHOUTBOX_USB_CHAT=$7
	local SHOUTBOX_USB_CHATFILE=$8
	...
	LIGHTTPD_ENV="setenv.add-environment = ( 
	   ...
	   \"SHOUTBOX_USB_CHAT\"      => \"$SHOUTBOX_USB_CHAT\" , 
	   \"SHOUTBOX_USB_CHATFILE\"  => \"$SHOUTBOX_USB_CHATFILE\" , 
	   $LIGHTTPD_ENV_BR_LINE 
...
generate_lighttpd_env $GLOBAL_CHAT "$GLOBAL_DEST" $PIRATEBOX_PYTHONPATH $GEN_CHATFILE $PIRATEBOX_FOLDER $CHATFILE $SHOUTBOX_USB_CHAT $SHOUTBOX_USB_CHATFILE
